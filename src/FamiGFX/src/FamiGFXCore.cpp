...
typedef struct fgfx_node {
    fgfx_handle_t handle;
    fgfx_handle_t parent;
    fgfx_handle_t first_child;
    fgfx_handle_t next_sibling;
    ...
    uint8_t dirty;
    fgfx_rect_t dirty_rect;
    uint8_t show_title;
    fgfx_window_border_t border;
    const char *title;
    fgfx_canvas_t *front_canvas;
    fgfx_canvas_t *back_canvas;
    uint8_t committed_dirty;
    fgfx_rect_t committed_rect;
    fgfx_draw_fn draw;
    fgfx_event_fn event;
    void *user;
} fgfx_node_t;

typedef struct fgfx_anim {
    fgfx_handle_t handle;
...
static void ctx_lock(fgfx_context_t *ctx)
{
    if (ctx && ctx->port) {
        ctx->port->lock();
    }
...
static void ctx_unlock(fgfx_context_t *ctx)
{
    if (ctx && ctx->port) {
        ctx->port->unlock();
    }
...
static void ctx_notify(fgfx_context_t *ctx)
{
    if (ctx && ctx->port) {
...
static fgfx_rect_t rect_make(int16_t x, int16_t y, int16_t w, int16_t h)
{
    fgfx_rect_t r;
    r.x = x;
    r.y = y;
...
static uint8_t rect_empty(fgfx_rect_t r)
{
    return r.w <= 0 || r.h <= 0;
}

static fgfx_rect_t rect_intersection(fgfx_rect_t a, fgfx_rect_t b)
{
...
static fgfx_rect_t rect_union(fgfx_rect_t a, fgfx_rect_t b)
{
    int16_t x1;
    int16_t y1;
    int16_t x2;
...
static fgfx_rect_t canvas_full_rect(const fgfx_canvas_t *canvas)
{
    if (!canvas) {
        return rect_make(0, 0, 0, 0);
    }
...
static fgfx_rect_t canvas_clip_rect(const fgfx_canvas_t *canvas)
{
    if (!canvas) {
        return rect_make(0, 0, 0, 0);
    }
...
{
    switch (format) {
    case FGFX_PIXEL_MONO1_LSB_VERTICAL:
        return (size_t)width * ((height + 7u) / 8u);
    case FGFX_PIXEL_GRAY4:
        return (size_t)((width + 1u) / 2u) * height;
    case FGFX_PIXEL_GRAY8:
...
fgfx_canvas_t *fgfx_canvas_create(uint16_t width, uint16_t height,
                                  fgfx_pixel_format_t format)
{
    fgfx_canvas_t *canvas = (fgfx_canvas_t *)calloc(1, sizeof(*canvas));
    size_t bytes = fgfx_canvas_required_bytes(width, height, format);
    ...
    if (format == FGFX_PIXEL_MONO1_LSB_VERTICAL) {
        canvas->stride = width;
    } else if (format == FGFX_PIXEL_GRAY4) {
        canvas->stride = (uint16_t)((width + 1u) / 2u);
    } else {
        canvas->stride = width;
    }
    fgfx_canvas_reset_clip(canvas);
    return canvas;
}
                               ...
                               fgfx_pixel_format_t format, uint8_t *pixels)
{
    ...
    } else if (format == FGFX_PIXEL_GRAY8) {
        min_stride = width;
    } else {
        return FGFX_ERR_INVALID_ARG;
    }
...
fgfx_result_t fgfx_canvas_intersect_clip(fgfx_canvas_t *canvas, fgfx_rect_t rect)
{
    ...
    return fgfx_canvas_set_clip(canvas, rect_intersection(canvas_clip_rect(canvas), rect));
}
...
fgfx_result_t fgfx_canvas_copy_rect(fgfx_canvas_t *dst, const fgfx_canvas_t *src,
                                    ...
                                    int16_t dst_y)
{
    fgfx_rect_t src_bounds;
    fgfx_rect_t src_clipped;
    int16_t y;
    ...
    for (y = 0; y < src_clipped.h; ++y) {
        ...
        for (x = 0; x < src_clipped.w; ++x) {
            ...
            fgfx_draw_pixel(dst, dx, dy, fgfx_get_pixel(src, sx, sy));
        }
    }
    return FGFX_OK;
}
                               ...
                               int16_t dst_x, int16_t dst_y)
{
    ...
    return fgfx_canvas_copy_rect(dst, src, canvas_full_rect(src), dst_x, dst_y);
}

static fgfx_node_t *find_node(fgfx_context_t *ctx, fgfx_handle_t handle)
{
    size_t i;
    if (!ctx || handle == FGFX_INVALID_HANDLE) {
        return NULL;
    }
...
static fgfx_rect_t node_full_dirty_rect(const fgfx_node_t *node)
{
    if (!node) {
        return rect_make(0, 0, 0, 0);
    }
...
static fgfx_rect_t normalize_node_dirty_rect(const fgfx_node_t *node,
                                             fgfx_rect_t rect)
{
    fgfx_rect_t full = node_full_dirty_rect(node);
    if (!node) {
        return full;
    }
    if (rect.w <= 0 || rect.h <= 0) {
...
static void mark_node_dirty_rect(fgfx_context_t *ctx, fgfx_node_t *node,
                                 fgfx_rect_t rect)
{
    if (!ctx || !node) {
        return;
...
static void mark_dirty_upwards(fgfx_context_t *ctx, fgfx_handle_t handle,
                               fgfx_rect_t rect)
{
    fgfx_node_t *node = find_node(ctx, handle);
    if (!ctx || !node) {
        return;
    }
    mark_node_dirty_rect(ctx, node, rect);
    while (node && node->parent != FGFX_INVALID_HANDLE) {
        fgfx_node_t *parent = find_node(ctx, node->parent);
        if (!parent) {
            break;
        }
...
static void mark_scene_dirty(fgfx_context_t *ctx)
{
...
static fgfx_result_t reserve_nodes(fgfx_context_t *ctx, size_t count)
{
    fgfx_node_t *nodes;
    size_t capacity;
    ...
    nodes = (fgfx_node_t *)realloc(ctx->nodes, capacity * sizeof(*nodes));
    if (!nodes) {
        return FGFX_ERR_NO_MEMORY;
    }
    ctx->nodes = nodes;
...
{
    fgfx_context_t *ctx = (fgfx_context_t *)calloc(1, sizeof(*ctx));
    fgfx_node_desc_t root;
    fgfx_handle_t ignored;
    ...
    root.kind = FGFX_NODE_LAYER;
    root.w = (int16_t)width;
    root.h = (int16_t)height;
    root.opacity = 255;
    root.visible = 1;
    root.owns_canvas = 0;
    if (fgfx_node_create(ctx, FGFX_INVALID_HANDLE, &root, &ignored) != FGFX_OK) {
        fgfx_destroy(ctx);
        return NULL;
    }
    return ctx;
...
static void destroy_owned_canvases(fgfx_context_t *ctx)
{
    size_t i;
    for (i = 0; i < ctx->node_count; ++i) {
        if (ctx->nodes[i].owns_canvas) {
            fgfx_canvas_destroy(ctx->nodes[i].front_canvas);
            fgfx_canvas_destroy(ctx->nodes[i].back_canvas);
        }
    }
}
...
static void append_child(fgfx_context_t *ctx, fgfx_handle_t parent,
                         fgfx_handle_t child)
{
    fgfx_node_t *p = find_node(ctx, parent);
    fgfx_node_t *tail;
    if (!p) {
        return;
    }
    if (p->first_child == FGFX_INVALID_HANDLE) {
        p->first_child = child;
        return;
    }
    tail = find_node(ctx, p->first_child);
    while (tail && tail->next_sibling != FGFX_INVALID_HANDLE) {
...
static void prepend_child(fgfx_context_t *ctx, fgfx_handle_t parent,
                          fgfx_handle_t child)
{
    fgfx_node_t *p = find_node(ctx, parent);
    fgfx_node_t *c = find_node(ctx, child);
    if (!p || !c) {
        return;
    }
...
static uint8_t node_is_descendant(fgfx_context_t *ctx, fgfx_handle_t node_handle,
                                  fgfx_handle_t ancestor_handle)
{
    fgfx_node_t *node = find_node(ctx, node_handle);
    while (node && node->parent != FGFX_INVALID_HANDLE) {
                               ...
                               fgfx_handle_t *out_handle)
{
    ...
    if (ctx->node_count > 0 && parent == FGFX_INVALID_HANDLE) {
        parent = ctx->nodes[0].handle;
    }
    if (ctx->node_count > 0 && !find_node(ctx, parent)) {
        return FGFX_ERR_NOT_FOUND;
    }
    ...
    if (fgfx_canvas_required_bytes(1, 1, node->canvas_format) == 0) {
        node->canvas_format = FGFX_PIXEL_GRAY8;
    }
    ...
    node->user = desc->user;
    node->next_sibling = FGFX_INVALID_HANDLE;
    node->first_child = FGFX_INVALID_HANDLE;
    node->dirty = 0;
    ...
    if (node->owns_canvas && node->rect.w > 0 && node->rect.h > 0) {
        node->front_canvas = fgfx_canvas_create((uint16_t)node->rect.w,
                                                (uint16_t)node->rect.h,
                                                node->canvas_format);
        node->back_canvas = fgfx_canvas_create((uint16_t)node->rect.w,
                                               (uint16_t)node->rect.h,
                                               node->canvas_format);
        if (!node->front_canvas || !node->back_canvas) {
            fgfx_canvas_destroy(node->front_canvas);
            fgfx_canvas_destroy(node->back_canvas);
            --ctx->node_count;
            return FGFX_ERR_NO_MEMORY;
        }
    }

    if (parent != FGFX_INVALID_HANDLE) {
...
static void detach_from_parent(fgfx_context_t *ctx, fgfx_node_t *node)
{
    fgfx_node_t *parent = find_node(ctx, node->parent);
    fgfx_node_t *prev = NULL;
    fgfx_node_t *cur;
    if (!parent) {
        return;
    }
    ...
    while (cur) {
        if (cur->handle == node->handle) {
            ...
            cur->next_sibling = FGFX_INVALID_HANDLE;
            return;
        }
        prev = cur;
...
static fgfx_result_t destroy_node_recursive(fgfx_context_t *ctx,
                                            fgfx_handle_t handle)
{
    size_t i;
    fgfx_node_t *node = find_node(ctx, handle);
    ...
    while (node->first_child != FGFX_INVALID_HANDLE) {
        fgfx_result_t res = destroy_node_recursive(ctx, node->first_child);
        if (res != FGFX_OK) {
            return res;
        }
...
fgfx_result_t fgfx_node_destroy(fgfx_context_t *ctx, fgfx_handle_t handle)
{
    ...
    parent = fgfx_node_parent(ctx, handle);
    res = destroy_node_recursive(ctx, handle);
    if (res == FGFX_OK && parent != FGFX_INVALID_HANDLE) {
...
{
    return fgfx_node_back_canvas(ctx, handle);
}
...
static fgfx_result_t resize_owned_node_canvas(fgfx_node_t *node,
                                              int16_t width,
                                              int16_t height)
{
    fgfx_canvas_t *new_front = NULL;
    fgfx_canvas_t *new_back = NULL;
    ...
    new_front = fgfx_canvas_create((uint16_t)width, (uint16_t)height,
                                   node->canvas_format);
    new_back = fgfx_canvas_create((uint16_t)width, (uint16_t)height,
                                  node->canvas_format);
    if (!new_front || !new_back) {
        fgfx_canvas_destroy(new_front);
        fgfx_canvas_destroy(new_back);
        return FGFX_ERR_NO_MEMORY;
    }
    fgfx_canvas_destroy(node->front_canvas);
    fgfx_canvas_destroy(node->back_canvas);
...
static void mark_node_committed_dirty(fgfx_context_t *ctx, fgfx_node_t *node,
                                           fgfx_rect_t rect)
{
    if (!ctx || !node) {
        return;
...
static fgfx_result_t commit_node_rect_locked(fgfx_context_t *ctx, fgfx_node_t *node,
                                                fgfx_rect_t rect)
{
    fgfx_rect_t saved_clip;
    fgfx_result_t res;
    ...
    fgfx_canvas_get_clip(node->front_canvas, &saved_clip);
    fgfx_canvas_reset_clip(node->front_canvas);
    res = fgfx_canvas_copy_rect(node->front_canvas, node->back_canvas,
                                rect, rect.x, rect.y);
    fgfx_canvas_set_clip(node->front_canvas, saved_clip);
    if (res == FGFX_OK) {
...
fgfx_result_t fgfx_node_discard(fgfx_context_t *ctx, fgfx_handle_t handle)
{
    ...
    fgfx_canvas_get_clip(node->back_canvas, &saved_clip);
    fgfx_canvas_reset_clip(node->back_canvas);
    res = fgfx_canvas_copy_rect(node->back_canvas, node->front_canvas,
                                node_full_dirty_rect(node), 0, 0);
    fgfx_canvas_set_clip(node->back_canvas, saved_clip);
    ctx_unlock(ctx);
    return res;
}
...
fgfx_result_t fgfx_node_bring_to_front(fgfx_context_t *ctx,
                                       fgfx_handle_t handle)
{
    fgfx_node_t *node = find_node(ctx, handle);
    fgfx_handle_t parent;
    if (!node || node->parent == FGFX_INVALID_HANDLE) {
        return FGFX_ERR_INVALID_ARG;
    }
    parent = node->parent;
    detach_from_parent(ctx, node);
...
fgfx_result_t fgfx_node_send_to_back(fgfx_context_t *ctx,
                                     fgfx_handle_t handle)
{
    fgfx_node_t *node = find_node(ctx, handle);
    fgfx_handle_t parent;
...
{
    fgfx_node_t *node = find_node(ctx, handle);
    fgfx_node_t *new_parent;
    fgfx_handle_t old_parent;
    if (!ctx || !node || handle == FGFX_INVALID_HANDLE ||
        handle == fgfx_root_handle(ctx) || parent == FGFX_INVALID_HANDLE ||
        handle == parent) {
        return FGFX_ERR_INVALID_ARG;
    }
    ...
    node->next_sibling = FGFX_INVALID_HANDLE;
    append_child(ctx, parent, handle);
    if (old_parent != FGFX_INVALID_HANDLE) {
                     ...
                     fgfx_color_t color)
{
    ...
    if (canvas->format == FGFX_PIXEL_MONO1_LSB_VERTICAL) {
    ...
    } else if (canvas->format == FGFX_PIXEL_GRAY4) {
    ...
    } else if (canvas->format == FGFX_PIXEL_GRAY8) {
...
void fgfx_clear(fgfx_canvas_t *canvas, fgfx_color_t color)
{
    ...
    if (clip.x == 0 && clip.y == 0 && clip.w == (int16_t)canvas->width &&
        ...
        if (canvas->format == FGFX_PIXEL_MONO1_LSB_VERTICAL) {
            memset(canvas->pixels, color ? 0xFF : 0x00,
                   fgfx_canvas_required_bytes(canvas->width, canvas->height,
                                              canvas->format));
            return;
        }
        if (canvas->format == FGFX_PIXEL_GRAY4) {
    ...
    fgfx_fill_rect(canvas, 0, 0, (int16_t)canvas->width,
                    ...
                    int16_t x1, int16_t y1, fgfx_color_t color)
{
    ...
    for (;;) {
        int16_t e2;
        fgfx_draw_pixel(canvas, x0, y0, color);
        if (x0 == x1 && y0 == y1) {
            break;
        }
        e2 = (int16_t)(2 * err);
...
void fgfx_draw_rect(fgfx_canvas_t *canvas, int16_t x, int16_t y,
                    int16_t w, int16_t h, fgfx_color_t color)
{
    fgfx_fill_rect(canvas, x, y, w, 1, color);
    fgfx_fill_rect(canvas, x, (int16_t)(y + h - 1), w, 1, color);
    fgfx_fill_rect(canvas, x, y, 1, h, color);
    fgfx_fill_rect(canvas, (int16_t)(x + w - 1), y, 1, h, color);
}
                      ...
                      int16_t r, fgfx_color_t color)
{
    ...
    int16_t x = 0;
    int16_t y = r;
    fgfx_draw_pixel(canvas, x0, (int16_t)(y0 + r), color);
    fgfx_draw_pixel(canvas, x0, (int16_t)(y0 - r), color);
    fgfx_draw_pixel(canvas, (int16_t)(x0 + r), y0, color);
    fgfx_draw_pixel(canvas, (int16_t)(x0 - r), y0, color);
    while (x < y) {
        if (f >= 0) {
            --y;
        ...
        ddx = (int16_t)(ddx + 2);
        f = (int16_t)(f + ddx);
        fgfx_draw_pixel(canvas, (int16_t)(x0 + x), (int16_t)(y0 + y), color);
        fgfx_draw_pixel(canvas, (int16_t)(x0 - x), (int16_t)(y0 + y), color);
        fgfx_draw_pixel(canvas, (int16_t)(x0 + x), (int16_t)(y0 - y), color);
        fgfx_draw_pixel(canvas, (int16_t)(x0 - x), (int16_t)(y0 - y), color);
        fgfx_draw_pixel(canvas, (int16_t)(x0 + y), (int16_t)(y0 + x), color);
        fgfx_draw_pixel(canvas, (int16_t)(x0 - y), (int16_t)(y0 + x), color);
        fgfx_draw_pixel(canvas, (int16_t)(x0 + y), (int16_t)(y0 - x), color);
        fgfx_draw_pixel(canvas, (int16_t)(x0 - y), (int16_t)(y0 - x), color);
    }
...
void fgfx_draw_triangle(fgfx_canvas_t *canvas, int16_t x0, int16_t y0,
                        ...
                        fgfx_color_t color)
{
    fgfx_draw_line(canvas, x0, y0, x1, y1, color);
    fgfx_draw_line(canvas, x1, y1, x2, y2, color);
    fgfx_draw_line(canvas, x2, y2, x0, y0, color);
}
                        ...
                        fgfx_color_t color)
{
    ...
    if (y0 == y2) {
        ...
        fgfx_fill_rect(canvas, a, y0, (int16_t)(b - a + 1), 1, color);
        return;
    }
    ...
    for (y = y0; y <= last; ++y) {
        ...
        fgfx_fill_rect(canvas, a, y, (int16_t)(b - a + 1), 1, color);
    }
...
static void draw_circle_corners(fgfx_canvas_t *canvas, int16_t x0, int16_t y0,
                                int16_t r, uint8_t corners, fgfx_color_t color)
{
    int16_t f = 1 - r;
    int16_t ddx = 1;
    int16_t ddy = (int16_t)(-2 * r);
                          ...
                          fgfx_color_t color)
{
    ...
    fgfx_fill_rect(canvas, (int16_t)(x + radius), y, (int16_t)(w - 2 * radius), 1, color);
    fgfx_fill_rect(canvas, (int16_t)(x + radius), (int16_t)(y + h - 1),
                   (int16_t)(w - 2 * radius), 1, color);
    fgfx_fill_rect(canvas, x, (int16_t)(y + radius), 1, (int16_t)(h - 2 * radius), color);
    fgfx_fill_rect(canvas, (int16_t)(x + w - 1), (int16_t)(y + radius), 1,
                          ...
                          fgfx_color_t color)
{
    ...
    for (iy = 0; iy < h; ++iy) {
        ...
        fgfx_fill_rect(canvas, (int16_t)(x + left), (int16_t)(y + iy),
                           ...
                           fgfx_color_t color, fgfx_color_t bg)
{
    ...
    for (yy = 0; yy < h; ++yy) {
        for (xx = 0; xx < w; ++xx) {
            ...
            if (on) {
                fgfx_draw_pixel(canvas, (int16_t)(x + xx), (int16_t)(y + yy), color);
            } else if (bg != color) {
                fgfx_draw_pixel(canvas, (int16_t)(x + xx), (int16_t)(y + yy), bg);
            }
        }
    }
}
                        ...
                        uint8_t size_x, uint8_t size_y)
{
    ...
    for (col = 0; col < 6; ++col) {
        ...
        for (row = 0; row < 8; ++row) {
            if (bits & (1u << row)) {
                fgfx_fill_rect(canvas, (int16_t)(x + col * size_x),
...
{
    while (text && *text) {
        fgfx_draw_char_5x7(canvas, x, y, *text++, color, bg, size_x, size_y);
                           ...
                           uint16_t *w, uint16_t *h)
{
    ...
    while (*text) {
        ...
        if (c == '\n') {
            ...
            const fgfx_glyph_t *glyph = &font->glyph[c - font->first];
...
static fgfx_color_t blend_over(fgfx_color_t dst, fgfx_color_t src, uint8_t opacity)
{
    int32_t diff = (int32_t)src - (int32_t)dst;
    return (fgfx_color_t)((int32_t)dst + (diff * opacity) / 255);
}

static int16_t title_bar_h(const fgfx_node_t *node)
{
...
static int16_t border_w(const fgfx_node_t *node)
{
    return node && node->border == FGFX_WINDOW_BORDER_RECT ? 1 : 0;
}

static int16_t outer_w(const fgfx_node_t *node)
{
    return (int16_t)(node->rect.w + border_w(node) * 2);
}

static int16_t outer_h(const fgfx_node_t *node)
{
    int16_t bw = border_w(node);
    return (int16_t)(node->rect.h + title_bar_h(node) +
                     (title_bar_h(node) ? bw : (int16_t)(bw * 2)));
}

static int16_t content_dx(const fgfx_node_t *node)
{
    return border_w(node);
}

static int16_t content_dy(const fgfx_node_t *node)
{
    int16_t th = title_bar_h(node);
    return th ? th : border_w(node);
}

static void draw_title_text_clipped(fgfx_canvas_t *canvas, int16_t x,
                                    int16_t y, int16_t max_w,
                                    const char *text, fgfx_color_t color)
{
    const fgfx_font_t *font = titleFont();
    while (canvas && font && text && *text && max_w >= font->glyph[0].width) {
        uint8_t ch = static_cast<uint8_t>(*text++);
        if (ch >= font->first && ch <= font->last) {
            const fgfx_glyph_t *glyph = &font->glyph[ch - font->first];
            if (glyph->xAdvance > max_w) {
                break;
            }
            fgfx_draw_char_font(canvas, font, x, y, ch, color, color, 1, 1);
...
static void draw_node_decoration(fgfx_canvas_t *target, const fgfx_node_t *node,
                                 int16_t x, int16_t y)
{
    int16_t bw = border_w(node);
    int16_t th = title_bar_h(node);
    ...
    if (th) {
        fgfx_fill_rect(target, x, y, ow, th, 255);
        draw_title_text_clipped(target, (int16_t)(x + 1), (int16_t)(y + 1),
                                (int16_t)(ow - 2), node->title, 0);
    }
    if (bw) {
        fgfx_draw_rect(target, x, y, ow, oh, 255);
    }
}

static void composite_canvas(fgfx_canvas_t *dst, const fgfx_canvas_t *src,
                             int16_t dx, int16_t dy, uint8_t opacity)
{
    fgfx_rect_t dst_clip;
    fgfx_rect_t src_area;
    int16_t y;
    ...
    for (y = 0; y < src_area.h; ++y) {
        ...
        for (x = 0; x < src_area.w; ++x) {
            ...
            fgfx_color_t s = fgfx_get_pixel(src, sx, sy);
            if (opacity == 255) {
                fgfx_draw_pixel(dst, dx_abs, dy_abs, s);
            } else {
                fgfx_color_t d = fgfx_get_pixel(dst, dx_abs, dy_abs);
                fgfx_draw_pixel(dst, dx_abs, dy_abs, blend_over(d, s, opacity));
            }
        }
    }
}

static void render_node(fgfx_context_t *ctx, fgfx_node_t *node,
                        fgfx_canvas_t *target, int16_t ox, int16_t oy);

static void render_children(fgfx_context_t *ctx, fgfx_node_t *node,
                            fgfx_canvas_t *target, int16_t ox, int16_t oy)
{
    fgfx_handle_t child;
    if (!ctx || !node || !target) {
    ...
    while (child != FGFX_INVALID_HANDLE) {
        fgfx_node_t *child_node = find_node(ctx, child);
        if (!child_node) {
            break;
        }
...
static void draw_dirty_node_callback(fgfx_context_t *ctx, fgfx_node_t *node)
{
    fgfx_rect_t dirty;
    fgfx_rect_t saved_clip;
    ...
    fgfx_canvas_get_clip(node->back_canvas, &saved_clip);
    fgfx_canvas_reset_clip(node->back_canvas);
    fgfx_canvas_set_clip(node->back_canvas, dirty);
    fgfx_clear(node->back_canvas, 0);
...
{
    fgfx_rect_t dirty;
    fgfx_rect_t saved_target_clip;
    fgfx_rect_t outer_clip;
    int16_t abs_x;
    int16_t abs_y;
    ...
    abs_x = (int16_t)(ox + node->rect.x);
    abs_y = (int16_t)(oy + node->rect.y);
    fgfx_canvas_get_clip(target, &saved_target_clip);
    outer_clip = rect_make(abs_x, abs_y, outer_w(node), outer_h(node));
    fgfx_canvas_intersect_clip(target, outer_clip);
    ...
    if (node->owns_canvas) {
        ...
        if (node->front_canvas) {
            composite_canvas(target, node->front_canvas,
                             ...
                             (int16_t)(abs_y + content_dy(node)), node->opacity);
        }
    ...
    fgfx_canvas_set_clip(target, saved_target_clip);
}
...
fgfx_result_t fgfx_render(fgfx_context_t *ctx, fgfx_canvas_t *target)
{
    ...
    ctx_lock(ctx);
    fgfx_canvas_get_clip(target, &saved_clip);
    fgfx_canvas_reset_clip(target);
    fgfx_clear(target, 0);
    child = ctx->nodes[0].first_child;
    while (child != FGFX_INVALID_HANDLE) {
        fgfx_node_t *node = find_node(ctx, child);
        if (!node) {
            break;
        }
    ...
    fgfx_canvas_set_clip(target, saved_clip);
    ctx->scene_dirty = 0;
    ctx_unlock(ctx);
    return FGFX_OK;
}
...
fgfx_result_t fgfx_daemon_step(fgfx_context_t *ctx)
{
    ...
    res = fgfx_render(ctx, &ctx->port->target);
    if (res != FGFX_OK) {
        return res;
    }
...
void fgfx_daemon_run(fgfx_context_t *ctx)
{
    ...
    while (ctx->daemon_running) {
         ...
         * has reached waitCommit(), and also lets structural changes made
         * during startup render immediately.
         */
        if (fgfx_needs_render(ctx)) {
            (void)fgfx_daemon_step(ctx);
            continue;
        }
...
fgfx_result_t fgfx_daemon_start(fgfx_context_t *ctx)
{
    ...
    ctx->daemon_running = 1;
    fgfx_result_t result = ctx->port->startDaemon((void (*)(void *))fgfx_daemon_run, ctx);
    if (result != FGFX_OK) {
...
#if FAMIGFX_ENABLE_POINTER_EVENTS
...
static fgfx_handle_t hit_test_node(fgfx_context_t *ctx, fgfx_node_t *node,
                                   int16_t x, int16_t y, int16_t ox,
                                   int16_t oy, int16_t *out_x,
                                   int16_t *out_y, uint8_t need_handler)
{
    fgfx_handle_t child;
    fgfx_handle_t hit = FGFX_INVALID_HANDLE;
    int16_t abs_x;
    int16_t abs_y;

    if (!node || !node->visible || node->opacity == 0) {
        return FGFX_INVALID_HANDLE;
    }
    ...
    while (child != FGFX_INVALID_HANDLE) {
        ...
        if (child_hit != FGFX_INVALID_HANDLE) {
            hit = child_hit;
        }
        child = child_node->next_sibling;
    }

    if (hit != FGFX_INVALID_HANDLE) {
        return hit;
    }
...
fgfx_handle_t fgfx_hit_test(fgfx_context_t *ctx, int16_t x, int16_t y)
{
    int16_t local_x;
    int16_t local_y;
    if (!ctx || ctx->node_count == 0) {
...
static uint8_t node_can_receive_event(const fgfx_node_t *node)
{
    return node && node->visible && node->opacity != 0 && node->event;
}

static fgfx_handle_t top_event_node(fgfx_context_t *ctx, fgfx_node_t *node)
{
    fgfx_handle_t child;
    fgfx_handle_t hit = FGFX_INVALID_HANDLE;
    if (!node || !node->visible || node->opacity == 0) {
    ...
    return node->event ? node->handle : FGFX_INVALID_HANDLE;
}

static fgfx_node_t *focused_event_node(fgfx_context_t *ctx)
{
    fgfx_node_t *node = find_node(ctx, ctx ? ctx->focus : FGFX_INVALID_HANDLE);
...
fgfx_result_t fgfx_focus_set(fgfx_context_t *ctx, fgfx_handle_t handle)
{
    ...
    if (handle != FGFX_INVALID_HANDLE && !find_node(ctx, handle)) {
        return FGFX_ERR_NOT_FOUND;
    }
    ctx->focus = handle;
    return FGFX_OK;
...
{
    return (ctx && find_node(ctx, ctx->focus)) ? ctx->focus : FGFX_INVALID_HANDLE;
}

static fgfx_handle_t focus_step(fgfx_context_t *ctx, int direction)
{
    size_t i;
    size_t start = 0;
    if (!ctx || ctx->node_count == 0) {
...
#if FAMIGFX_ENABLE_POINTER_EVENTS
uint8_t fgfx_dispatch_pointer(fgfx_context_t *ctx, fgfx_event_type_t type,
                              ...
                              fgfx_handle_t *out_target)
{
    ...
    fgfx_node_t *node;
    if (out_target) {
        *out_target = FGFX_INVALID_HANDLE;
    }
    if (!ctx || ctx->node_count == 0 ||
        (type != FGFX_EVENT_POINTER_DOWN && type != FGFX_EVENT_POINTER_UP &&
         type != FGFX_EVENT_POINTER_MOVE)) {
        return 0;
    }
                            ...
                            fgfx_handle_t *out_target)
{
    ...
    if (event->type == FGFX_EVENT_KEY_DOWN || event->type == FGFX_EVENT_KEY_UP ||
        event->type == FGFX_EVENT_CANCEL) {
...
#if FAMIGFX_ENABLE_POINTER_EVENTS
    if (event->type == FGFX_EVENT_POINTER_DOWN ||
        event->type == FGFX_EVENT_POINTER_UP ||
        event->type == FGFX_EVENT_POINTER_MOVE) {
        return fgfx_dispatch_pointer(ctx, event->type, event->x, event->y,
                          ...
                          fgfx_handle_t *out_target)
{
    fgfx_event_t event;
    memset(&event, 0, sizeof(event));
    event.type = pressed ? FGFX_EVENT_KEY_DOWN : FGFX_EVENT_KEY_UP;
    event.key = key;
    event.repeat = repeat ? 1u : 0u;
    event.modifiers = modifiers;
    return fgfx_dispatch_event(ctx, &event, out_target);
}

static fgfx_result_t reserve_anims(fgfx_context_t *ctx, size_t count)
{
    fgfx_anim_t *anims;
    size_t capacity;
    ...
    anims = (fgfx_anim_t *)realloc(ctx->anims, capacity * sizeof(*anims));
    if (!anims) {
        return FGFX_ERR_NO_MEMORY;
    }
    ctx->anims = anims;
...
static int32_t ease_value(const fgfx_anim_t *anim)
{
    uint32_t t;
    int32_t delta;
    ...
    t = (anim->elapsed_ms * 1024u) / anim->duration_ms;
    if (anim->curve == FGFX_ANIM_EASE_OUT) {
        t = 1024u - (((1024u - t) * (1024u - t)) / 1024u);
    }
    delta = anim->to - anim->from;
    return anim->from + (int32_t)((delta * (int32_t)t) / 1024);
}
...
void fgfx_tick(fgfx_context_t *ctx, uint32_t delta_ms)
{
    ...
    for (i = 0; i < ctx->anim_count; ++i) {
        fgfx_anim_t *anim = &ctx->anims[i];
        fgfx_node_t *node = find_node(ctx, anim->handle);
        int32_t value;
        if (!anim->active || !node) {
            continue;
        }
        ...
        if (anim->property == FGFX_ANIM_X) {
            node->rect.x = (int16_t)value;
        } else if (anim->property == FGFX_ANIM_Y) {
            node->rect.y = (int16_t)value;
        } else if (anim->property == FGFX_ANIM_OPACITY) {
